"""Modulo de visualizacion del tablero en pygame."""

import pygame


class TableroVisual:
    """Clase que dibuja el tablero y elementos visuales del juego."""

    def __init__(self, ancho=1400, alto=700):
        """Inicializa el tablero visual con dimensiones especificadas."""
        self.ancho = ancho
        self.alto = alto
        ...
        self.color_movimiento_posible = (0, 255, 0, 100)  # Verde semitransparente
        ...
        # banco
        self.rect_banco_blancas = pygame.Rect(
            self.barra_x, 0, self.ancho_barra, self.alto // 2 - 25
        )
        self.rect_banco_negras = pygame.Rect(
            self.barra_x, self.alto // 2 + 25, self.ancho_barra, self.alto // 2 - 25
        )

        # boton de tirar dados (en la barra)
        self.rect_boton_dados = pygame.Rect(
            self.barra_x + 5, self.alto // 2 - 45, self.ancho_barra - 10, 35
        )
    ...
    def _crear_mapa_visual(self):
        """Crea el mapeo de posiciones logicas a coordenadas visuales."""
        mapa = {}
    ...
    def dibujar_ficha(self, pantalla, x, y, color):
        """Dibuja una ficha en la posicion especificada."""
    ...
    def dibujar_fichas_en_triangulo(
        self, pantalla, triangulo_x, triangulo_y, ancho_triangulo, cantidad, color, hacia_abajo=True
    ):
        """Dibuja multiples fichas apiladas en un triangulo."""
    ...
    def dibujar_fichas_en_barra(self, pantalla, cantidad, color, zona_rect):
        """Dibuja fichas en el banco central."""
    ...
    def dibujar_fichas_retiradas(self, pantalla, cantidad, color, zona_rect):
        """Dibuja fichas retiradas en la zona lateral."""
        if cantidad == 0:
            return
    ...
    def dibujar_texto(self, pantalla, texto, pos_centro, fuente, color):
        """Dibuja texto centrado en una posicion."""
    ...
    def dibujar_superficie_alpha(self, pantalla, rect, color):
        """Dibuja un rectangulo semitransparente."""
        s = pygame.Surface((rect.width, rect.height), pygame.SRCALPHA)
        s.fill(color)
        pantalla.blit(s, rect.topleft)

    def dibujar(
        self, pantalla, estado_juego, origen_seleccionado, mensaje_ui, movimientos_posibles=None
    ):
        """Dibuja el tablero completo con toda la informacion del juego."""
        ...
        for pos_logica, fichas in enumerate(tablero_logico):
            if fichas:
                cantidad = len(fichas)
                color = (
                    self.color_ficha_blanca if fichas[0] == "Blancas" else self.color_ficha_negra
                )
                x, y, hacia_abajo, rect = self.mapa_posiciones_visuales[pos_logica]
                self.dibujar_fichas_en_triangulo(
                    pantalla, x, y, self.espacio_lado, cantidad, color, hacia_abajo
                )

        banco = estado_juego["banco"]
        if banco["Blancas"] > 0:
            self.dibujar_fichas_en_barra(
                pantalla, banco["Blancas"], self.color_ficha_blanca, self.rect_banco_blancas
            )
        if banco["Negras"] > 0:
            self.dibujar_fichas_en_barra(
                pantalla, banco["Negras"], self.color_ficha_negra, self.rect_banco_negras
            )

        fichas_retiradas = self._contar_fichas_retiradas(tablero_logico, banco)
        self.dibujar_fichas_retiradas(
            pantalla, fichas_retiradas["Negras"], self.color_ficha_negra, self.rect_retiro_negras
        )
        self.dibujar_fichas_retiradas(
            pantalla, fichas_retiradas["Blancas"], self.color_ficha_blanca, self.rect_retiro_blancas
        )
        ...
        self.dibujar_texto(
            pantalla, "TIRAR", self.rect_boton_dados.center, self.fuente_ui, (255, 255, 255)
        )
        # Turno
        ...
        self.dibujar_texto(
            pantalla,
            turno_texto,
            (self.barra_x + self.ancho_barra // 2, 20),
            self.fuente_ui,
            self.color_fondo,
        )
        # Dados
        ...
        self.dibujar_texto(
            pantalla,
            dados_texto,
            (self.barra_x + self.ancho_barra // 2, self.alto // 2 + 60),
            self.fuente_dados,
            self.color_ficha_blanca,
        )
        # Mensaje
        ...
        self.dibujar_texto(
            pantalla, mensaje_ui, (self.ancho // 2, self.alto - 25), self.fuente_error, color_msg
        )
        # Ganador
        if estado_juego["ganador"]:
            ...
            self.dibujar_texto(
                ...
                pygame.font.SysFont("Arial", 70, bold=True),
                (255, 215, 0),
            )
        if movimientos_posibles:
            for origen, destinos in movimientos_posibles.items():
                for destino in destinos:
                    if destino == "retirar":
                        if estado_juego["ficha actual"] == "Blancas":
                            self.dibujar_superficie_alpha(
                                pantalla, self.rect_retiro_blancas, self.color_movimiento_posible
                            )
                        else:
                            self.dibujar_superficie_alpha(
                                pantalla, self.rect_retiro_negras, self.color_movimiento_posible
                            )
                    elif isinstance(destino, int) and destino in self.mapa_posiciones_visuales:
                        rect_destino = self.mapa_posiciones_visuales[destino][3]
                        self.dibujar_superficie_alpha(
                            pantalla, rect_destino, self.color_movimiento_posible
                        )

        if origen_seleccionado is not None:
            if origen_seleccionado == "banco":
                zona_banco = (
                    self.rect_banco_blancas
                    if estado_juego["ficha actual"] == "Blancas"
                    else self.rect_banco_negras
                )
    ...
    def _contar_fichas_retiradas(self, tablero, banco):
        """Cuenta las fichas retiradas del juego."""
        fichas_blancas_tablero = sum(1 for pos in tablero if pos and "Blancas" in pos for _ in pos)
        fichas_negras_tablero = sum(1 for pos in tablero if pos and "Negras" in pos for _ in pos)

        return {
            "Blancas": 15 - fichas_blancas_tablero - banco["Blancas"],
            "Negras": 15 - fichas_negras_tablero - banco["Negras"],
        }

    def convertir_clic_a_posicion(self, pos_clic):
        """Convierte coordenadas de clic en posicion logica del tablero."""
        for pos_logica, (x, y, ha, rect) in self.mapa_posiciones_visuales.items():
            if rect.collidepoint(pos_clic):
                return pos_logica

        if self.rect_banco_blancas.collidepoint(pos_clic) or self.rect_banco_negras.collidepoint(
            pos_clic
        ):
            return "banco"

        if self.rect_retiro_blancas.collidepoint(pos_clic) or self.rect_retiro_negras.collidepoint(
            pos_clic
        ):
            return "retiro"
